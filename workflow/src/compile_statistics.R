...
evaluations <- foreach::foreach(i=seq_len(nrow(qMatrix)), .combine='rbind', .inorder=T) %dopar% {
    ...
    if(file.exists(tfile)){
        ...
        if(any(nrow(dt) == 0)){
            evaluations <- data.frame(transcription_factor = qi[1], tissue = qi[2], type = qi[3], auc = NA, var = NA, low = NA, upp = NA, num_0 = NA, num_1 = NA)
        } else if (any(nrow(dt) > 0)) {
            evaluations <- calculate_metrics(dt, qi[1], qi[2], qi[3])
            evaluations <- evaluations %>% as.data.frame() %>%
                stats::setNames(c('transcription_factor', 'tissue', 'type', 'auc', 'var', 'low', 'upp'))
