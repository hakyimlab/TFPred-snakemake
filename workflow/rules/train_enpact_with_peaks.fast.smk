...
rule evaluate_Enpact:
    ...
    shell:
        ...
        if [[ {params.nlines} -lt 100 ]]; then
            ...
            {params.rscript} workflow/src/evaluate_enet.R --logistic_model {input.logistic_model} --train_data_file {input.train_data} --test_data_file {input.test_data} --eval_output {params.basename}
        fi
        """
