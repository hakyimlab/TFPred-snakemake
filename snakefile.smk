...
HOMERFILES_DIR = config['training']['motif_files_directory'] #os.path.join(DATA_DIR, 'homer_instances')
...
if config['training']['peaks_only'] == False:
...
if config['run_enformer'] == True:
    ...
    if config['training']['peaks_only'] == True:
        print("INFO - Training on ChIP peaks only...")
        include: 'workflow/rules/train_enpact_with_peaks.fast.smk'
    else:
